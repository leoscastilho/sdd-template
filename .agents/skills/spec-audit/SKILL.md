...
2. Launch the `spec_auditor` agent for the semantic sweep (contradictions, stale statements, specs marked `implemented` whose criteria lack tests). It returns quoted evidence; you open each cited location and keep, downgrade or drop the finding. Only adjudicated findings go to the user.
