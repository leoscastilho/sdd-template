---
name: idea
description: Discuss a feature and capture it as a structured brief in docs$ideas/, checked against accepted ADRs and every "Ruled out" list, shaped for $spec-new to convert. Use for "$idea", "let's talk through a feature", "write this discussion up as a brief". Suitable for short, dictated conversations.
---
...
This may be run from a phone, with the input **dictated** rather than typed. **Keep replies short.** One question at a time, not a wall of them.
...
- `AGENTS.md` → "Architecture constraints you must hold".
...
A voice conversation elsewhere may have had no repository access and may not have checked the specs. If the user pastes such a transcript:
