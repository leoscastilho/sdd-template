---
name: spec-sync
description: Reconcile specs after code changed — find every changed code file, its governing spec, and update the spec so it describes current behaviour. Use when the Stop hook or `make spec-drift` reports drift, after scaffolding or `prisma generate`, or for "/spec-sync".
---

# /spec-sync

1. Run `make spec-drift` (working tree), or read the hook output you were given.
2. For each **stale** entry (`file -> spec`): decide what the code now does that the spec does not say (use `log-summarizer` for large diffs). Then brief the `spec-editor` agent per spec: the paragraph, criteria, endpoints, config keys or ruled-out entries to update and the exact facts to state, plus the `last_reviewed` bump. If nothing behavioural changed, the brief is only the bump; say so in your report. Read the editor's diff before continuing.
