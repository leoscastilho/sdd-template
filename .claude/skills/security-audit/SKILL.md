...
The orchestrator runs this; the `security-auditor` agent does the reading. Governing spec: ARCH-SECURITY "Continuous security audit".
...
## 3. Semantic layer
Launch `security-auditor` (Opus) with: the scope list, the categories, and the stack hints from the architecture specs and ADRs under `specs/`. For `full` scope (or more than ~40 files) launch **five auditors in parallel, one per category**, in a single message; for `diff`/`staged`/small scopes launch **one auditor with all five**. Merge results; de-duplicate on `file:line`.
